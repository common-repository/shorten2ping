...
Description: Sends <strong>status</strong> updates to Ping.fm everytime you publish a post, using your own domain, bit.ly, wp.me, su.pr, is.gd and others for shortened permalinks.
...
// new method for facebook ??                  
//echo '<meta property="og:title" content="'.get_bloginfo('name').'" />';
//echo '<meta property="og:url" content="'.get_bloginfo('url').'" />';
//echo '<meta property="og:description" content="'.get_bloginfo('description').'" />';                  
//echo '<meta property="og:image" content="'.$thumbnail_permalink.'" />';  
...
function short_url_head() {
    ...
    function fb_thumb_in_head() {
     ...
     global $post;
          ...
          // se supone que es la nueva forma de decirle la imagen, pero no valida el xthml
          //        echo '<meta property="og:image" content="'.$thumbnail_permalink.'" />';                     
...
function shorten2ping_published_post($post) {
    ...
    $pingfm_user_email = $s2p_options['pingfm_email'];
    ...
    $short_url_exists = get_post_meta($post_id, 'short_url', true);
            ...
            } elseif ($s2p_options['ping_service'] == 'pingfm-email') {
            
               send_pingfm_email($pingfm_user_email,$post_id,$message);
...
// create options array. if options already exists add_option function does nothing.
  ...
  $s2p_options['s2p_ver'] = "147";      

     // check if plugin installed is previous to 1.4.6, and add new options in that case

  $existing_s2p_options = get_option('s2p_options');

  $s2p_version = $existing_s2p_options['s2p_ver'];  
  ...
  if (empty($existing_s2p_options)) {
     ...
     } elseif (empty($s2p_version)) {

          unset($existing_s2p_options[twitter_user]);
          unset($existing_s2p_options[twitter_pass]);
          unset($existing_s2p_options[trim_user]);
          unset($existing_s2p_options[trim_pass]);
          
          $existing_s2p_options[ping_service] = 'pingfm';
          
          $s2p_new_options = array ("s2p_ver" => "146");
          $merged_options = array_merge($existing_s2p_options, $s2p_new_options);
          
          delete_option ('s2p_options');
          add_option('s2p_options', $merged_options );  
     } elseif ($s2p_version == '146') {
     
          $existing_s2p_options[s2p_ver] = '147';
          
          $s2p_new_options = array ("pingfm_email" => "");
          $merged_options = array_merge($existing_s2p_options, $s2p_new_options);     
...
<div class="wrap">
	 ...
	 <h2><?php _e('Shorten2Ping Options','shorten2ping') ?></h2>
		...
		<p><?php _e('Shorten2Ping allows you to update status at Ping.fm whenever a new blog entry is published.  To start using it, simply enter the required information below, and press update information button. You only need to fill data for the services you want to use.','shorten2ping') ?>
    </p><p>
		<?php _e('You can also customize the message for the status notification by using the "message" field below.  You can use [title] to represent the title of the blog entry, [link] to represent the permalink, and [tags] to convert post tags into Twitter hashtags.','shorten2ping') ?>
    </p>
  ...
  <form method="post" name="options" action="options.php">
    ...
    <?php
     ...
     if (!empty($s2p_wp_blog_id)) { ?>
    <input type="hidden" name="s2p_options[wp_blog_id]" value="<?php echo $s2p_wp_blog_id; ?>" />
    <?php } else { ?>
    <input type="hidden" name="s2p_options[wp_blog_id]" value="" />
    <?php } ?>   
    
    <input type="hidden" name="s2p_options[s2p_ver]" value="146" />
    ...
    <div id="tabs-1">    
      ...
      <td class="description"><?php _e('You can use [title] for the post title, [link] for the short link, and [tags] to use posts tags as Twitter hashtags. Example: New post, [title] [link] [tags]','shorten2ping') ?>
      </td>
      </tr>        
      ...
      <td><select name="s2p_options[ping_service]">
	         <option value='pingfm' <?php if ($s2p_options['ping_service'] == 'pingfm') echo 'selected="selected"'; ?> ><?php _e('Ping.fm user key','shorten2ping') ?></option>
	         <option value='pingfm-email' <?php if ($s2p_options['ping_service'] == 'pingfm-email') echo 'selected="selected"'; ?> ><?php _e('Ping.fm email','shorten2ping') ?></option>
	         <option value='none' <?php if ($s2p_options['ping_service'] == 'none') echo 'selected="selected"'; ?>><?php _e('None','shorten2ping') ?></option>
           </select></td>
      <td class="description"><?php _e('Choose to send notification to Ping.fm using user key, posting email, or turn off this feature.','shorten2ping') ?></td>
      </tr>
    ...
    <div id="tabs-2">
      ...
      <td><?php _e('API Key','shorten2ping') ?> <input type="text" class="widefat" name="s2p_options[pingfm_key]" value="<?php echo($s2p_options['pingfm_key']); ?>" />
      <?php _e('Posting email','shorten2ping') ?> <input type="text" class="widefat" name="s2p_options[pingfm_email]" value="<?php echo($s2p_options['pingfm_email']); ?>" />
      </td>
      <td class="description"><?php _e('Put your Ping.fm <a href="http://ping.fm/key/">API key</a> here or your "secret" <a href="http://ping.fm/email/">posting email</a>.','shorten2ping') ?></td>
      </tr>
...
function send_pingfm_email($pingfm_user_email,$post_id,$message) {
	if (!$pingfm_user_email) return false;
  ...
  $headers .= "From: $admin_email";
   ...
   if (!mail($pingfm_user_email, $subject, $message, $headers)) {

       add_post_meta($post_id, 'pingfm_email', 'Email not sent, check settings and requirements on your host.');
